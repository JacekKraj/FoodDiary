...
describe('changing date', () => {
  ...
  it("shows spinner on clicking change date arrow when this day's content wasn't downloaded yet", () => {
    const wrapper = setup({
      diary: { currentDate: getModifiedDate(), currentDiary: sampleDiary, downloadedDiary: {} },
    });
  ...
  it("doesn't show spinner on clicking change date arrow when this day's conntent already exists in downloaded", () => {
    const wrapper = setup({
      diary: { currentDate: getModifiedDate(), currentDiary: sampleDiary, downloadedDiary: sampleDiary },
    });
    const spinner = simulateClicking(wrapper);
    expect(spinner.exists()).toBe(true);
  });
  it("displays new day's products and new slider's values", () => {
    ...
    const wrapper = setup({ diary: { currentDate: '2021-09-01', currentDiary: diary, downloadedDiary: diary } });
...
describe('clicking remove content button', () => {
  ...
  beforeEach(() => {
    wrapper = setup({ diary: { currentDate: getModifiedDate(), currentDiary: sampleDiary, downloadedDiary: sampleDiary } });
